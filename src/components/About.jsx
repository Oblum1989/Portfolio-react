import React from 'react';
import '../assets/styles/components/About.css';

const About = () => (
  <div id="tf-about">
    <div className="overlay">
      <div className="container">
        <div className="row">
          <div className="col-md-6 col-md-offset-6">
            <h3>About Me</h3>
            
            <div className="text-profile">
              <p>Ingeniero de Sistemas con grandes habilidades en desarrollo de software enfocado en Ruby
                On Rails. Visionario y apasionado por la codificación, entusiasta
                del aprendizaje continuo, con facilidades de adaptación y comunicación.</p>
              <p>Con experiencia desarrollando habilidades en la solución a todo tipo
                de problemas planteados en el manejo y manipulación de la información aplicando análisis,
                diseño y
                construcción de soluciones informáticas que se ajusten a las necesidades, con conocimientos
                en
                Ruby, JavaScript, HTML5, CSS3, Bootstrap 4 y Ruby on Rails, planeación y ejecución de
                proyectos. Servicios
                profesionales en mantenimiento de redes de telecomunicaciones tecnologías GSM, UMTS, LTE.
              </p>
            </div>
            
            <a href="#tf-education" className="btn btn-primary my-btn dark">Studies</a>
          </div>
        </div>
      </div>
    </div>
  </div>
);

export default About;